...
function checkForAppts() {
  return fetch(url)
    .then(response => {
      ...
      const appts = $('.calendar-table .calendar-month-table table').find('td.buchbar ');
      ...
      if (!appts.length) {
        ...
        const apptLinks = appts.map((a, appt) => {
          return {
            link: `https://service.berlin.de/terminvereinbarung/termin/` + $(appt).find('a').prop('href'),
            text:  appts.find('span').text() + ' ' +  appts.parents('tbody').siblings('thead').find('.month').text()
          }
        })
        ...
        var data = {
          ...
          to: emails,
          subject: 'APPT FOUND some ' + i,
          html: email(apptLinks.toArray())
        };
...
checkForAppts()
setInterval(checkForAppts, 60 * 1000 * Math.floor((Math.random() + 1) * (10 - 1)) + 1)
